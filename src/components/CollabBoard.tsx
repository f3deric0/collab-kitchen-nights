...
import {
  ChefHat, Plus, Users, ShoppingBag, ArrowRight,
  Sparkles, X, Calendar, MessageCircle, Heart
} from "lucide-react";
...
const COLLAB_IDEAS = [
  { emoji: "🍝", idea: "Pasta battle", desc: "Ognuno porta una sua ricetta di pasta" },
  ...
  { emoji: "🥘", idea: "Curry collettivo", desc: "Spezie da tutto il mondo in un piatto" },
  { emoji: "🍕", idea: "Pizza fatta in casa", desc: "Impasto a mano, farciture libere" },
...
const CollabBoard = () => {
  ...
  // Estrai "manca X" dalle note
  const getMissing = (notes: string | null) => {
    if (!notes) return null;
    const patterns = [/manca[no]?\s+([^,.!?]+)/i, /serve\s+([^,.!?]+)/i, /cerco\s+([^,.!?]+)/i, /missing\s+([^,.!?]+)/i];
  ...
  // Estrai idea principale dalle note (prima riga / prima frase)
  const getIdea = (notes: string | null) => {
    if (!notes) return null;
    const cleaned = notes.replace(/manca[no]?\s+[^,.!?]+/gi, "").replace(/JOIN REQUEST[^)]+\)/gi, "").trim();
    return cleaned.split(/[.!?]/)[0].trim() || null;
  };

  const handlePropose = async () => {
    if (!propName || !propEmail || !propDate) {
      toast.error("Inserisci nome, email e data.");
      return;
    }
    ...
    try {
      ...
      toast.success("Collab proposta! Apparirà nella bacheca 🍳");
      setShowProposeForm(false);
      setPropName(""); setPropEmail(""); setPropDate(""); setPropIdea(""); setPropMissing("");
  ...
  const handleJoin = async () => {
    if (!joinName || !joinEmail || !joiningCollab) {
        ...
        <div className="mx-auto max-w-5xl">
          ...
          >
            ...
            <p className="mx-auto mt-4 max-w-xl font-body text-base leading-relaxed text-muted-foreground">
              Qualcuno ha già un'idea ma gli manca un ingrediente — o un compagno di cucina.
              Sfoglia le proposte e unisciti, oppure proponi la tua collab.
            </p>
          </motion.div>
          ...
          >
            <div className="flex gap-3 pb-2">
              <span className="shrink-0 self-center font-body text-xs font-bold uppercase tracking-[0.2em] text-muted-foreground">
                Idee settimana prossima →
              </span>
          ...
          {loading ? (
            <div className="grid gap-4 sm:grid-cols-2">
              {[1, 2].map(i => (
                <div key={i} className="h-40 animate-pulse rounded-[1.75rem] border border-border bg-muted/40" />
              ))}
            </div>
          ) : proposals.length === 0 ? (
            <motion.div
              ...
              {proposals.map((p, i) => {
                ...
                return (
                  ...
                  >
                    ...
                    {idea && (
                      <p className="mb-3 font-body text-sm italic text-muted-foreground line-clamp-2">
                        "{idea}"
                      </p>
                    )}
                    ...
                    <button
                      onClick={() => { setJoiningCollab(p); setShowJoinForm(true); }}
          ...
          >
            ...
            >
              ...
              Proponi la tua collab
            </button>
          </motion.div>
        </div>
      </section>

      {/* ── Modal: Proponi collab ── */}
      <AnimatePresence>
        {showProposeForm && (
          <motion.div
            initial={{ opacity: 0 }}
            ...
            >
              <div className="mb-5 flex items-start justify-between">
                <div>
                  ...
                  <p className="mt-1 font-body text-sm text-muted-foreground">Descrivi la tua idea e cosa manca — gli altri potranno unirti.</p>
                </div>
                <button onClick={() => setShowProposeForm(false)} className="rounded-full p-2 text-muted-foreground hover:bg-muted">
                  <X className="h-4 w-4" />
                </button>
              </div>
              ...
              <div className="space-y-4">
                <div className="grid grid-cols-2 gap-3">
                  <div>
                    ...
                    <input value={propName} onChange={e => setPropName(e.target.value)} placeholder="Federico"
                      className="w-full rounded-xl border border-input bg-background px-3 py-2.5 font-body text-sm text-foreground placeholder:text-muted-foreground focus:outline-none focus:ring-2 focus:ring-accent/40" />
                  </div>
                  <div>
                    <label className="mb-1.5 block font-body text-xs font-semibold text-foreground">Data *</label>
                    <input type="date" value={propDate} min={todayStr} onChange={e => setPropDate(e.target.value)}
                      className="w-full rounded-xl border border-input bg-background px-3 py-2.5 font-body text-sm text-foreground focus:outline-none focus:ring-2 focus:ring-accent/40" />
                  </div>
                </div>

                <div>
                  <label className="mb-1.5 block font-body text-xs font-semibold text-foreground">Email *</label>
                  <input type="email" value={propEmail} onChange={e => setPropEmail(e.target.value)} placeholder="tua@email.com"
                    className="w-full rounded-xl border border-input bg-background px-3 py-2.5 font-body text-sm text-foreground placeholder:text-muted-foreground focus:outline-none focus:ring-2 focus:ring-accent/40" />
                </div>
                ...
                <div>
                  ...
                  <textarea value={propIdea} onChange={e => setPropIdea(e.target.value)} rows={2}
                    ...
                    className="w-full resize-none rounded-xl border border-input bg-background px-3 py-2.5 font-body text-sm text-foreground placeholder:text-muted-foreground focus:outline-none focus:ring-2 focus:ring-accent/40" />
                </div>

                <div>
                  <label className="mb-1.5 block font-body text-xs font-semibold text-foreground">
                    <ShoppingBag className="mr-1 inline h-3.5 w-3.5 text-orange-500" />
                    Manca qualcosa? (opzionale)
                  </label>
                  <input value={propMissing} onChange={e => setPropMissing(e.target.value)}
                    placeholder="Es: guanciale, pasta fresca, vino bianco…"
                    className="w-full rounded-xl border border-input bg-background px-3 py-2.5 font-body text-sm text-foreground placeholder:text-muted-foreground focus:outline-none focus:ring-2 focus:ring-accent/40" />
                  <p className="mt-1 font-body text-xs text-muted-foreground">Apparirà come badge sulla tua card — gli altri possono offrirsi di portarlo</p>
                </div>
              </div>
          ...
          >
            ...
            >
              <div className="mb-5 flex items-start justify-between">
                ...
                <button onClick={() => { setShowJoinForm(false); setJoiningCollab(null); }}
              ...
              {getMissing(joiningCollab.notes) && (
                <div className="mb-4 rounded-[1.2rem] border border-orange-400/30 bg-orange-400/8 px-4 py-3">
                  <p className="font-body text-sm">
                    <span className="font-semibold">🛒 Portate con voi:</span>{" "}
              ...
              <div className="space-y-4">
                ...
                <div>
                  <label className="mb-1.5 block font-body text-xs font-semibold text-foreground">Email *</label>
                  <input type="email" value={joinEmail} onChange={e => setJoinEmail(e.target.value)} placeholder="tua@email.com"
                    className="w-full rounded-xl border border-input bg-background px-3 py-2.5 font-body text-sm text-foreground placeholder:text-muted-foreground focus:outline-none focus:ring-2 focus:ring-accent/40" />
                </div>
                <div>
                  <label className="mb-1.5 block font-body text-xs font-semibold text-foreground">
                    <ShoppingBag className="mr-1 inline h-3.5 w-3.5 text-accent" />
                    Cosa porti? (opzionale)
                  </label>
                  <input value={joinBrings} onChange={e => setJoinBrings(e.target.value)}
                    placeholder="Es: guanciale, vino, dessert…"
                    className="w-full rounded-xl border border-input bg-background px-3 py-2.5 font-body text-sm text-foreground placeholder:text-muted-foreground focus:outline-none focus:ring-2 focus:ring-accent/40" />
                </div>
              </div>
              ...
              <p className="mt-3 text-center font-body text-xs text-muted-foreground">
                Chicco riceverà la tua richiesta e vi mette in contatto.
              </p>
            </motion.div>
          </motion.div>
        )}
